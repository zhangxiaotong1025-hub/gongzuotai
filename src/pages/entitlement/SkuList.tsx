...
import { skuData as initialData, appData, BILLING_CYCLES, STATUS_MAP, getApp, getProduct, type Sku } from "@/data/entitlement";
...
export default function SkuList() {
  ...
  const columns: TableColumn<Sku>[] = [
    ...
    { key: "productIds", title: "关联权益产品", minWidth: 180, render: (_v: unknown, row) => {
      const products = ((row as Sku).productIds || []).map((pid) => getProduct(pid)).filter(Boolean);
      return products.length > 0 ? (
        <div className="flex flex-wrap gap-1">
          {products.slice(0, 2).map((p) => <span key={p!.id} className="badge-info">{p!.name}</span>)}
          {products.length > 2 && <span className="text-[11px] text-muted-foreground">+{products.length - 2}</span>}
        </div>
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="商品SKU" subtitle="可售卖的商品单元，每个商品通过关联权益产品形成售卖方案" actions={
