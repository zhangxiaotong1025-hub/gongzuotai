...
import { capabilityData as initialData, appData, ruleData, DATA_TYPES, DATA_TYPE_MAP, STATUS_MAP, type Capability, type DataType } from "@/data/entitlement";
      ...
      <div className="relative w-full max-w-[560px] rounded-xl border bg-card p-0 animate-in fade-in-0 zoom-in-95 duration-200 overflow-hidden" style={{ boxShadow: "var(--shadow-md)" }}>
        <div className="border-b bg-muted/40 px-5 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[13px] text-muted-foreground">定义平台可售卖的技术服务能力单元</p>
          </div>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <select className="filter-input w-full" value={form.dataType} onChange={(e) => {
                ...
                setForm({ ...form, dataType: dt, unit: DATA_TYPES.find((t) => t.value === dt)?.unit || "" });
              }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input className="filter-input w-full font-mono text-[12px]" value={form.apiPath} onChange={(e) => setForm({ ...form, apiPath: e.target.value })} />
            </div>
          </div>
          <div className="space-y-1.5">
...
export default function CapabilityList() {
  ...
  const columns: TableColumn<Capability>[] = [
    ...
    { key: "appName", title: "所属应用", minWidth: 140, render: (v, row) => <button className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-primary/10 text-primary hover:bg-primary/20 transition-colors" onClick={() => navigate(`/entitlement/app/detail/${(row as Capability).appId}`)}>{v}</button> },
    ...
    { key: "unit", title: "单位", minWidth: 60 },
    {
      key: "id", title: "规则数", minWidth: 70, align: "center" as const,
      render: (_v, row) => {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="权益能力管理" subtitle="定义平台可售卖的技术服务能力单元，每个能力归属一个应用" actions={
