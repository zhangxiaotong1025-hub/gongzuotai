...
import { Network, Layers, Shield, Table as TableIcon, GitBranch } from "lucide-react";

type Node = { layer: string; key: string; label: string; role: string; inputs: string[]; outputs: string[]; forbidden: string };

const NODES: Node[] = [
  { layer: "L1 边界层", key: "APP", label: "应用 App", role: "权益隔离的最大边界，多租户切片的根",
    inputs: ["接入登记（域名 + 业务 API 列表）"], outputs: ["AppCreated 事件", "RLS tenant key"],
    forbidden: "禁止跨 App 共享额度账目；禁止用 App 表达「功能开关」" },
  { layer: "L2 技术层", key: "CAP", label: "能力 Capability", role: "能力点 + API 绑定 + 数据类型 + 消耗系数",
    inputs: ["业务 API 路径", "数据类型 COUNTER/BOOLEAN/STORAGE/DURATION"], outputs: ["可被规则引用的能力句柄"],
    forbidden: "禁止把业务流程写在能力里；禁止持有客户状态" },
  { layer: "L2 业务层", key: "RUL", label: "规则 Rule", role: "额度 + 周期 + 派生策略，最小可发放单元",
    inputs: ["能力 + 额度 + periodType"], outputs: ["派生 grantType / expirePolicy"],
    forbidden: "禁止用户单独覆盖派生策略（后端强制 deriveRulePolicy）" },
  { layer: "L3 封装层", key: "PRD", label: "权益产品 Product", role: "面向交易的封装，关联多 Rule（N:M）",
    inputs: ["规则集合", "兑换方式 paid/credit/free"], outputs: ["ProductPublished 事件"],
    forbidden: "已被在售 SKU 引用时禁止删除规则关联" },
  { layer: "L3 销售层", key: "SKU", label: "商品 SKU / 套餐", role: "可独立售卖单元，含价格 + 计费周期",
    inputs: ["权益产品（仅 paid+active）", "价格 / 计费周期"], outputs: ["SkuOnSale 事件"],
    forbidden: "在售商品价格变更走审批；禁止跳过权益产品直挂规则" },
  { layer: "L4 履约层", key: "ORD", label: "订单 Order", role: "三维状态机：审核 / 支付 / 生命周期",
    inputs: ["客户 + 商品 + 幂等键"], outputs: ["Order* 事件族", "履约信号 Grant"],
    forbidden: "不允许跨域直写账户；状态变更必须通过状态机" },
  { layer: "L4 履约层", key: "ACC", label: "账户 Account", role: "客户聚合视图，跨应用持有 account_capability",
    inputs: ["来自订单的发放信号"], outputs: ["AccountAggregated 事件", "可消耗额度"],
    forbidden: "禁止旁路状态机直写额度；禁止跳过 sourceOrderIds 写入" },
  { layer: "L5 数据层", key: "EVT", label: "事件总线 Kafka", role: "Outbox + Relay 至少投递一次",
...
export default function Blueprint() {
  return (
    <div className="space-y-10">
      ...
      <section id="bp-arch" className="scroll-mt-4">
        <H icon={Network}>2.1 全量架构图 · 5 层 + 双视角</H>
        <Mermaid
  ...
  class EVT,BI data`}
      ...
      <section id="bp-nodes" className="scroll-mt-4">
        <H icon={TableIcon}>2.2 节点全表 · 输入 / 输出 / 禁区（一览，无需点击）</H>
        <div className="overflow-hidden rounded-xl border">
          <table className="w-full text-[12px]">
            <thead className="bg-muted/40 text-foreground/80">
              <tr>
                <th className="text-left px-3 py-2 font-medium w-[90px]">层级</th>
                <th className="text-left px-3 py-2 font-medium w-[160px]">节点</th>
                <th className="text-left px-3 py-2 font-medium">职责定义</th>
                <th className="text-left px-3 py-2 font-medium w-[24%]">输入</th>
                <th className="text-left px-3 py-2 font-medium w-[24%]">输出</th>
                <th className="text-left px-3 py-2 font-medium w-[24%]">禁区</th>
              </tr>
            </thead>
            <tbody className="[&_tr]:border-t align-top">
              {NODES.map(n => (
                <tr key={n.key} className="hover:bg-muted/20">
                  <td className="px-3 py-2 font-mono text-[10.5px] uppercase tracking-wider text-muted-foreground">{n.layer}</td>
                  <td className="px-3 py-2">
                    <div className="text-[12.5px] font-semibold text-primary">{n.label}</div>
                    <div className="font-mono text-[10px] text-muted-foreground mt-0.5">{n.key}</div>
                  </td>
                  <td className="px-3 py-2 text-foreground/85">{n.role}</td>
                  <td className="px-3 py-2">
                    <ul className="list-disc pl-4 space-y-0.5 text-foreground/85">
                      {n.inputs.map(i => <li key={i}>{i}</li>)}
                    </ul>
                  </td>
                  <td className="px-3 py-2">
                    <ul className="list-disc pl-4 space-y-0.5 text-emerald-700/90">
                      {n.outputs.map(o => <li key={o}>{o}</li>)}
                    </ul>
                  </td>
                  <td className="px-3 py-2 text-amber-700/90">{n.forbidden}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section id="bp-trade" className="scroll-mt-4">
        <H icon={Shield}>2.3 四种交易模式 · 同一组三维状态机的 4 种入口</H>
        <Mermaid
  ...
  G --> EV["Kafka EntitlementGranted"]`}
        ...
        <div className="mt-3 overflow-hidden rounded-xl border">
          <table className="w-full text-[12.5px]">
            <thead className="bg-muted/40 text-foreground/80">
              <tr>
                <th className="text-left px-3 py-2 font-medium">交易模式</th>
                <th className="text-left px-3 py-2 font-medium">触发场景</th>
                <th className="text-left px-3 py-2 font-medium">audit 初值</th>
                <th className="text-left px-3 py-2 font-medium">payment 初值</th>
                <th className="text-left px-3 py-2 font-medium">是否需对账</th>
              </tr>
            </thead>
            <tbody className="[&_td]:px-3 [&_td]:py-2 [&_tr]:border-t">
              <tr><td>user_purchase</td><td>用户在店内自付下单</td><td>auto_approved</td><td>pending → paid</td><td>是 · 财务月结</td></tr>
              <tr><td>credit</td><td>积分 / 优惠券兑换</td><td>auto_approved</td><td>no_payment</td><td>积分日终</td></tr>
              <tr><td>internal_grant</td><td>运营 / 客成赠送</td><td>pending_audit</td><td>no_payment</td><td>否 · 审计</td></tr>
              <tr><td>enterprise_grant</td><td>企业入驻自动派生</td><td>follow_enterprise</td><td>no_payment</td><td>跟随母单</td></tr>
            </tbody>
          </table>
        </div>
      </section>

      <section id="bp-view" className="scroll-mt-4">
        <H icon={Layers}>2.4 双视角访问控制（平台 / 企业）</H>
      ...
      <section id="bp-rule" className="scroll-mt-4">
        <H icon={GitBranch}>2.5 关键架构约束（Inviolable Rules）</H>
